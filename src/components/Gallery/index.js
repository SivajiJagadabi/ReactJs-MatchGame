...
class Gallery extends Component {
  ...
  checkImgMatching = () => {
    const {activeImageId, selectImageId} = this.state
    const imageLength = imagesList.length
    const index = Math.ceil(Math.random() * (imageLength - 1))
    const imgObj = imagesList[index]
    const image = imgObj.imageUrl
    const requireImageId = imgObj.id

    if (activeImageId === selectImageId) {
      this.setState(prevState => ({score: prevState.score + 1}))
      this.setState({selectedImage: image, selectImageId: requireImageId})
    } else {
      this.stopTimer()
      this.setState({endGame: true})
    }
  ...
  onClickImageItem = imageId => {
    this.setState({activeImageId: imageId}, this.checkImgMatching)
  }
  ...
  renderGallery = () => {
    ...
    return (
      <div className="bg-container">
        ...
        <img src={selectedImage} alt="match" className="image" />
        ...
        <div className="score-card-bg">
          <div className="score-card">
            ...
            <p className="score-text">
              YOUR SCORE <br /> <span className="score-span">{score}</span>
            </p>
            <button
              type="button"
  ...
  render() {
    ...
    return <> {endGame ? this.renderScoreCard() : this.renderGallery()} </>
  }
}
